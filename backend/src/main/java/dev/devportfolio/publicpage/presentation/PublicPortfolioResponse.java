...
public record PublicPortfolioResponse(PublicProfileResponse profile, List<PublicExperienceResponse> experiences,
    ...
    public static PublicPortfolioResponse from(PublicPortfolioView view, Locale locale) {
        ...
        return new PublicPortfolioResponse(
                PublicProfileResponse.from(view.profile(), socialLinks, locale),
                view.experiences().stream()
                        .map(experience -> PublicExperienceResponse.from(experience, skillsById, locale))
                        .toList(),
                view.projects().stream().map(project -> PublicProjectResponse.from(project, skillsById, locale))
                        .toList(),
                view.skills().stream().map(PublicSkillResponse::from).toList(),
                view.educations().stream().map(education -> PublicEducationResponse.from(education, locale))
                        .toList(),
